...
void ssd1306_drawLetter(unsigned char x, unsigned char y,unsigned char c){
    unsigned char c1 = c - 32;
    
    for(int i = 0; i < 5; i++){
        int col = ASCII[c1][i];
        for(int j = 0; j < 8; j++){
            ssd1306_drawPixel((x+i),(y+j),(1 & col >> j));
        }
    }
}
...
void ssd1306_drawMessage(unsigned char x, unsigned char y,char* message){
    ...
    while(1){
        ...
        ssd1306_drawLetter((x + 6*i),y,message[i]);
        //ssd1306_update();
        i++;
    }
}
