...
#define LOWTIME 12 // number of 48MHz cycles to be low for 0.35uS
#define HIGHTIME 62 // number of 48MHz cycles to be high for 1.65uS
...
void ws2812b_setColor(wsColor * c, int numLEDs) {
    ...
    volatile unsigned int delay_times[1 + 2*3*8 * numLEDs]; // I only gave you 5 WS2812B, adjust this if you get more somewhere
    ...
    for (i = 1; i < numBits; i++) {
        ...
        LATBINV = 0b10000000; // invert B7
    }
    LATBbits.LATB7 = 0;
    TMR2 = 0;
    if (nB = 1+ 2*3*8*numLEDs){
        NU32DIP_YELLOW = 0;
    }
    while(TMR2 < 2500){} // wait 50uS, reset condition
}
